...
public:
    int findDuplicate(vector<int>& nums) {
        ...
        int curr = 0;
        for(int i = 0; i < nums.size(); i++) { // if perfect loop, should only take nums.size() tries
        ...
        return nums[0]; // means that one of the duplicates is stuck at index 0.
        // It fails the prev_node != prev check because both will be 0, so this is the fallback.
    }
};
