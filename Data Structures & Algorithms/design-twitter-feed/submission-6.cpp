...
public:
    ...
    vector<int> getNewsFeed(int userId) {
        ...
        while(!pq.empty()) {
            ...
            if(v[2] > 0) { // add tweets if there exists more tweets by that user
                //this is the only way to enforce N*logN time where N is the number of users
                // cant process all tweets in the PQ because that is M log M time where M is number of tweets
