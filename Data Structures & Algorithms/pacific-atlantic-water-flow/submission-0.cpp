...
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        vector<vector<bool>> visitedp(heights.size(), vector<bool>(heights[0].size()));
        vector<vector<bool>> visiteda(heights.size(), vector<bool>(heights[0].size()));
        ...
        while(!dfs.empty()) {
            auto p = dfs.top();
            dfs.pop();
            if(visitedp[p.first][p.second]) {continue;}
            visitedp[p.first][p.second] = true;
        ...
        while(!dfs.empty()) {
            auto p = dfs.top();
            dfs.pop();
            if(visiteda[p.first][p.second]) {continue;}
            visiteda[p.first][p.second] = true;
